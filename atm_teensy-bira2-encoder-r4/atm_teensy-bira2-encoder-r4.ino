...
void setup() {
   ...
   btn2.begin(5);
   //btn2.onPress2( volMaster, volMaster.EVT_BTN_1);
   btn3.begin(8);
   encBtn1.begin(1);
   encBtn2.begin(2);
   displayMain.begin();
...
void loop() {
  ...
  if (enc_button_counter_1 == 0){
      btn3.onPress( wav1, wav1.EVT_WAV_OFF ); //control bira 1
      }
  else if (enc_button_counter_1 == 1) {
      btn3.onPress( wav2, wav2.EVT_WAV_OFF );
      }
//switch which volume the encoder controls
  
  if ( paramTimer.state() == 0 ) {
	  if (enc_button_counter_2 == 0){						//trigger btn_1 on volume
		  btn2.onPress( volMaster, volMaster.EVT_BTN_1 );
	  }
	  else if (enc_button_counter_2 == 1) {						
		  btn2.onPress( volWav1, volWav1.EVT_BTN_1 );
	  }
	  else if (enc_button_counter_2 == 2) {
		  btn2.onPress( volWav2, volWav2.EVT_BTN_1 );
	  }
  }
	  ...
	  if (enc_button_counter_2 == 0){					
		enc2.onChange( ATM_UP, volMaster, volMaster.EVT_ENC_UP );
	    enc2.onChange( ATM_DOWN, volMaster, volMaster.EVT_ENC_DOWN );
		btn2.onPress2( 1, volMaster, volMaster.EVT_BTN_1 );		
